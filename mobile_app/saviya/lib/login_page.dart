    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          Container(color: const Color(0x7A262525)),
        ...
        onPressed: () async {
          ...
          if (email.isNotEmpty && password.isNotEmpty) {
            try {
              ...
              await prefs.setString('last_name', response['user']['last_name']);
              ...
              await prefs.setBool(
                'is_rejected',
                response['user']['is_rejected'] == true,
              );

              print('Loading user data from SharedPreferences...');
              print('userid: ${prefs.getInt('userid')}');
              print('first_name: ${prefs.getString('first_name')}');
              print('is_verified: ${prefs.getBool('is_verified')}');

              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    'Login Successful! Welcome, ${response['user']['first_name']}',
                  ),
                ),
              );
